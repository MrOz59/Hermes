...
**Where the work lives.** Everything described in this file is committed to
`main`. Foundation, Tier 1 and Tier 2 have shipped in a release (v0.5.1 and
earlier). Everything from "Isolation and multi-user" through "Game Mode" is
newer: it sits under [Unreleased] in the changelog and has not shipped. Read
every `[x]` in those sections as "written and tested", not as "released".
...
incompatibly. The caveats and the two unchecked items in this section are not
polish - they are the reasons it cannot be recommended yet.
...
- [~] An audio sink per session, so a client hears its own session and the host
  hears the host. True for a session that runs as the Hermes user; a session
  with a Unix account of its own gets an audio server Hermes cannot reach
  through its `0700` runtime directory, so that session still hears the host —
  and says so in the log.
- [~] Remote Input is rejected while independent sessions are enabled: an
  input-only connection has no session of its own to attach to, so the request
  is refused instead of landing in somebody's desktop.
...
- [x] Verified end to end: `scripts/session-broker-test.sh`, 44 checks, all
  ...
  real units, so it only runs there. A full desktop and simultaneous real
...
- [~] **The session renders in software.** This is the honest state of the
  ...
  node Mesa can drive, or none at all — not anything in Hermes. The render node
  handed to a session compositor is now chosen the way the capture path chooses
  one, and a candidate must survive an allocation probe before it is believed;
...
- [~] The driver still marks two modes preferred — its synthetic EDID timing and
      ...
      larger. Every profile works around it by naming the mode. The fix exists
      on the driver's `main` (advertise one preferred mode), but no release
      carries it, so the workaround stays until one does.
- [ ] Driver release state is behind the host: the last release is still v0.3.2.
      Driver `main` is at 0.4.0 with UAPI v13, the one-preferred-mode fix and
      configfs card creation, but none of it is released and installed modules
      are test builds. Everything the host can now use depends on a driver users
      cannot yet install.

## Capture coverage per compositor
...
- [~] `ext-image-copy-capture` implemented and verified against labwc, but it does
      **not** deliver what the #29 report claimed it would: KWin 6.7.4 does not
      ...
      What this backend buys is GNOME and the compositors that never spoke
      screencopy. Where a compositor offers both, screencopy stays the default;
      `HERMES_WAYLAND_CAPTURE=icc` forces the new one, which is how it is tested.
...
- **The release gap.** Everything in this file is on `main`, but the work from
  "Isolation and multi-user" onward has not shipped: it sits under [Unreleased]
  in the changelog, and the driver surface it uses — UAPI v13, the
  one-preferred-mode fix, configfs card creation — is unreleased at driver
  0.4.0. Deciding what the next release contains, with the driver release
  preceding the host one, is the prerequisite for this roadmap meaning anything.
